...
$.ajax({
	type: 'GET',
	url: '/HeartToNerve/get-userlikedsongs?userId=' + userId,
	dataType: 'json',
	data: {
		userId : userId
	},
	success: function (songs) {
		...
		for (let i = 0; i < tracks.length; i++) {
			...
			const trackArtistNameCell = document.createElement("td");
			const trackAlbumNameCell = document.createElement("td");
			...
			trackTitleCell.textContent = tracks[i].songTitle;
			trackDurationCell.textContent = tracks[i].duration;
			trackArtistNameCell.textContent = tracks[i].artistName;
			trackAlbumNameCell.textContent = tracks[i].albumName;

			playButtonCell.appendChild(playSongButton);
			trackRow.appendChild(trackNumberCell);
			trackRow.appendChild(trackTitleCell);
			trackRow.appendChild(trackArtistNameCell);
			trackRow.appendChild(trackAlbumNameCell);
		...
		const songInfoProgress =(i) =>{
			...
			heartLi.id = 'btn' + i;
				...
				pTag.textContent = tracks[i].songTitle;
				...
				data: { userId: userId, songId: tracks[i].songId },
				dataType: 'json',
		...
		heartLi.addEventListener('click', function() {
						...
						if (isFavourite) {
							...
							$.ajax({
								...
								data: { userId: userId, songId: tracks[currentPlayingIndex].songId },
								dataType: 'json',
								success: function() {
							...
							console.log(tracks[currentPlayingIndex].songId);
